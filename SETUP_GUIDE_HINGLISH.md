...
## PART 4B: Bina Docker ke chalana (sab kuch laptop pe) 💻

Docker install nahi karna? Tab bhi project chalega. Bas judge **local mode** mein chalega.

> ⚠️ **Local mode mein sandbox isolation NAHI hota.** Code seedha tumhare laptop pe chalta hai (time limit lagti hai, lekin memory/network limit nahi). Sirf apne aur trusted doston ke code ke liye use karo. Public deploy hamesha Docker mode mein karna.

**Install karo (ek baar):**
| Cheez | Kahan se | Dhyan rakho |
|---|---|---|
| Python 3.12 | https://www.python.org/downloads/ | Installer mein **"Add python.exe to PATH"** ✅ tick karna |
| Node.js 22 LTS | https://nodejs.org | |
| PostgreSQL 16 | https://www.postgresql.org/download/windows/ | Install ke time `postgres` user ka password yaad rakhna |
| Redis (Windows ke liye Memurai) | https://www.memurai.com/get-memurai → **Developer** edition (free) | Windows service ban ke port 6379 pe chalta hai. Har 10 din mein restart chahiye |

Mac pe: `brew install python@3.12 node postgresql@16 redis` phir `brew services start postgresql@16 redis`.

**Database banao (ek baar):** Start menu se **"SQL Shell (psql)"** kholo, Enter dabate jao, `postgres` ka password daalo, phir:
```sql
CREATE USER codeclash WITH PASSWORD 'codeclash';
CREATE DATABASE codeclash OWNER codeclash;
```
...
Terminal 2, judge (local mode):
```powershell
cd judge
pip install -r requirements.txt
$env:JUDGE_MODE="local"     # PowerShell. CMD mein: set JUDGE_MODE=local   Mac: export JUDGE_MODE=local
python worker.py
```
...
| Error | Solution |
|---|---|
| `venv\Scripts\activate` pe "running scripts is disabled" | PowerShell mein ek baar: `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` |
| `python` not found | Python dobara install karo, "Add to PATH" tick karke. Ya `py` command use karo |
| Backend: "Postgres/Redis not reachable" | Services chal rahi hain? `services.msc` mein **postgresql** aur **Memurai** "Running" hone chahiye |
| `password authentication failed` | psql wala step dobara karo, ya `backend/.env` mein likho: `DATABASE_URL=postgresql+asyncpg://codeclash:TUMHARA_PASSWORD@localhost:5432/codeclash` |
| Submit karne pe "Judging..." atka hai | Judge terminal mein `mode=local` dikhna chahiye. `$env:JUDGE_MODE="local"` set karna bhool gaye honge |
